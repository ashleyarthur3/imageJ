...
DIR <- getwd()
EXPT <- dirname(getwd())
EXPTNAME <- basename(getwd())

#make new directory for trimmed newzeroscans in experiment folder
dir.create(file.path(EXPT, paste(EXPTNAME,"linescans_trim")))
DIR_linescans <- file.path(EXPT, paste(EXPTNAME,"linescans_trim"))

list <- list.dirs(path = ".", full.names = FALSE, recursive = FALSE) #list all directories
slices <- list[grep("sliced", list)] #list only slices

list <- list.files(path = ".", full.names = FALSE, recursive = TRUE) #list all directories+files
slices <- list[grep("linescan", list)] #list only sliced directories containing linescan files
slices <- dirname(slices) #save only directory names
slices <- slices[!duplicated(slices)] #remove duplications

for (i in 1:length(slices)){ #cycle through sliced folders
  setwd(file.path(DIR,slices[i]))
  o = file_path_sans_ext(basename(slices[i])) #save name of sliced directory
  files <- list.files(path=".", full.names = FALSE, recursive = FALSE) #list all files in subdir
  scans <- files[grep("linescan", files)] #save only linescans
  
  for (j in 1:length(scans)) { #go through all saved linescans
    p = file_path_sans_ext(basename(scans[j])) #save name of file
    df <- read.delim(scans[j], header=F) #read in file
    colnames(df) <- c("Microns","Intensity") #change column names
    df <- subset(df, Intensity <60000) #subset to exclude cell body
    write.csv(df, file=paste(DIR_linescans,"/",o,"_",p,"_trim.csv", sep=""), row.names = F) #save trimmed file
  }
  setwd(file.path(DIR))
}
...
for (fileName in fileNames) {
  ...
  i = 1
  write.csv(dp[[i]], file=paste(DIROUT_single_IN,p,"_single.csv", sep=""), row.names = F) #changed _ to / in paste
}
...
for (fileName in fileNames) {
  ...
  output = tapply(Values[["Y"]], cut(Values[["X"]], 6), median)
